<?php
/**
 * Test Auto Increment cho bảng maintenance_cases
 * File: test_maintenance_cases_auto_increment.php
...
echo "<h2>Test Auto Increment cho bảng maintenance_cases</h2>";
...
try {
    ...
    $stmt = $pdo->query("SELECT MAX(id) as max_id, MIN(id) as min_id, COUNT(*) as total_records FROM maintenance_cases");
    $result = $stmt->fetch();
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as count FROM maintenance_cases WHERE id = 0");
    $result = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("SELECT id, case_code, created_at FROM maintenance_cases WHERE case_code = ?");
    $stmt->execute([$testCode]);
    ...
    if ($testRecord) {
        ...
        echo "<li>Mã: " . $testRecord['case_code'] . "</li>";
    ...
    error_log("Error in test_maintenance_cases_auto_increment.php: " . $e->getMessage());
}
