<?php
/**
 * Test Auto Increment cho bảng maintenance_requests
 * File: test_maintenance_auto_increment.php
...
echo "<h2>Test Auto Increment cho bảng maintenance_requests</h2>";
...
try {
    ...
    $stmt = $pdo->query("SELECT MAX(id) as max_id, MIN(id) as min_id, COUNT(*) as total_records FROM maintenance_requests");
    $result = $stmt->fetch();
    ...
    $stmt = $pdo->query("SELECT COUNT(*) as count FROM maintenance_requests WHERE id = 0");
    $result = $stmt->fetch();
    ...
    $stmt = $pdo->prepare("SELECT id, request_code, created_at FROM maintenance_requests WHERE request_code = ?");
    $stmt->execute([$testCode]);
    ...
    if ($testRecord) {
        ...
        echo "<li>Mã: " . $testRecord['request_code'] . "</li>";
    ...
    error_log("Error in test_maintenance_auto_increment.php: " . $e->getMessage());
}

echo "<hr>";
echo "<p><a href='maintenance_requests.php'>← Quay lại trang Yêu cầu bảo trì</a></p>";
echo "<p><a href='fix_maintenance_requests_auto_increment.php'>🔧 Chạy fix auto-increment</a></p>";
?>
