<?php
/**
 * API Fix Auto Increment cho bảng maintenance_tasks
 * File: api/fix_maintenance_tasks_auto_increment.php
...
try {
    ...
    if (!$idColumn) {
        throw new Exception("Không tìm thấy cột 'id' trong bảng maintenance_tasks");
    }
    ...
    $stmt = $pdo->query("SELECT MAX(id) as max_id FROM maintenance_tasks");
    $result = $stmt->fetch();
    $maxId = $result['max_id'];
    ...
    if (strpos($idColumn['Extra'], 'auto_increment') === false) {
        $alterSql = "ALTER TABLE maintenance_tasks MODIFY COLUMN id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY";
        $pdo->exec($alterSql);
    ...
    $resetSql = "ALTER TABLE maintenance_tasks AUTO_INCREMENT = $nextId";
    $pdo->exec($resetSql);
    ...
    if ($current_user_id) {
        $log_message = "Fix auto increment cho bảng maintenance_tasks - Từ: " . ($currentAutoIncrement ?: 'NULL') . " -> " . $newAutoIncrement;
        $log_sql = "INSERT INTO user_activity_logs (user_id, activity, details, ip_address) VALUES (?, ?, ?, ?)";
        $log_stmt = $pdo->prepare($log_sql);
        $log_stmt->execute([
            $current_user_id,
            'FIX_MAINTENANCE_TASKS_AUTO_INCREMENT',
            $log_message,
    ...
    error_log("Database error in fix_maintenance_tasks_auto_increment.php: " . $e->getMessage());
    ...
    error_log("Error in fix_maintenance_tasks_auto_increment.php: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false, 
