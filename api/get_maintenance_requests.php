...
try {
    ...
    (
        SELECT COUNT(*) FROM maintenance_tasks dt 
        INNER JOIN maintenance_cases dc ON dt.maintenance_case_id = dc.id 
        WHERE dc.maintenance_request_id = dr.id
    ) as total_tasks,
    (
        SELECT COUNT(*) FROM maintenance_cases dc WHERE dc.maintenance_request_id = dr.id AND dc.status = 'Hoàn thành'
    ) as completed_cases
