...
try {
    ...
    // Cập nhật case
    $sql = "UPDATE maintenance_cases SET " . implode(', ', $updates) . " WHERE id = ?";
    $stmt = $pdo->prepare($sql);
    $stmt->execute($params);
    
    // Ghi log
    $log_message = "Cập nhật maintenance case: " . $current_case['case_code'];
    $log_stmt = $pdo->prepare("
        INSERT INTO user_activity_logs (user_id, activity, details, created_at) 
        VALUES (?, ?, ?, NOW())
    ");
    $log_stmt->execute([$current_user_id, 'update_maintenance_case', $log_message]);
    
    echo json_encode([
        'success' => true,
        'message' => 'Cập nhật maintenance case thành công'
    ]);
