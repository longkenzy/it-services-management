<?php
/**
 * API Fix Auto Increment cho bảng maintenance_requests
 * File: api/fix_maintenance_requests_auto_increment.php
...
try {
    ...
    $stmt = $pdo->query("DESCRIBE maintenance_requests");
    ...
    if (!$idColumn) {
        throw new Exception("Không tìm thấy cột 'id' trong bảng maintenance_requests");
    }
    ...
    $stmt = $pdo->query("SHOW TABLE STATUS LIKE 'maintenance_requests'");
    ...
    $stmt = $pdo->query("SELECT MAX(id) as max_id FROM maintenance_requests");
    $result = $stmt->fetch();
    $maxId = $result['max_id'];
    ...
    if (strpos($idColumn['Extra'], 'auto_increment') === false) {
        $alterSql = "ALTER TABLE maintenance_requests MODIFY COLUMN id int(11) NOT NULL AUTO_INCREMENT PRIMARY KEY";
        $pdo->exec($alterSql);
    ...
    $resetSql = "ALTER TABLE maintenance_requests AUTO_INCREMENT = $nextId";
    $pdo->exec($resetSql);
    ...
    $stmt = $pdo->prepare("INSERT INTO maintenance_requests (request_code, customer_id, sale_id, maintenance_status, created_by) VALUES (?, 1, 1, 'Tiếp nhận', 1)");
    $stmt->execute([$testCode]);
    
    $testId = $pdo->lastInsertId();
    
    // Xóa record test
    $stmt = $pdo->prepare("DELETE FROM maintenance_requests WHERE request_code = ?");
    $stmt->execute([$testCode]);
    ...
    if ($current_user_id) {
        $log_message = "Fix auto increment cho bảng maintenance_requests - Từ: " . ($currentAutoIncrement ?: 'NULL') . " -> " . $newAutoIncrement;
        $log_sql = "INSERT INTO user_activity_logs (user_id, activity, details, ip_address) VALUES (?, ?, ?, ?)";
        $log_stmt = $pdo->prepare($log_sql);
        $log_stmt->execute([
            $current_user_id,
            'FIX_MAINTENANCE_REQUESTS_AUTO_INCREMENT',
            $log_message,
    ...
    error_log("Database error in fix_maintenance_requests_auto_increment.php: " . $e->getMessage());
    ...
    error_log("Error in fix_maintenance_requests_auto_increment.php: " . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false, 
