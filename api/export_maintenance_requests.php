 ...
 * File: api/export_maintenance_requests.php
 * Purpose: Export maintenance requests data to Excel format
 */
...
try {
    // Xây dựng câu query để lấy dữ liệu maintenance requests
    $sql = "SELECT 
                mr.id,
                mr.request_code,
                mr.po_number,
                mr.contract_type,
                mr.request_detail_type,
                mr.email_subject_customer,
                mr.email_subject_internal,
                mr.expected_start,
                mr.expected_end,
                mr.contact_person,
                mr.contact_phone,
                mr.requester_notes,
                mr.maintenance_manager,
                ...
                (
                    ...
                    INNER JOIN maintenance_cases mc ON mt.maintenance_case_id = mc.id 
                    WHERE mc.maintenance_request_id = mr.id
                ) as total_tasks
    ...
    $sheet->setTitle('Danh sách Yêu cầu bảo trì');
    ...
    foreach ($requests as $index => $request) {
        ...
        $data = [
            ...
            $request['maintenance_manager'] ?: 'N/A',
        ...
        foreach ($data as $colIndex => $value) {
            ...
            if ($colIndex == 15) { // Cột trạng thái bảo trì
                $sheet->getStyle($column . $row)->applyFromArray([
                    'fill' => [
                        'fillType' => Fill::FILL_SOLID,
    ...
    $filename = "Danh_sach_Yeu_cau_bao_tri_{$timestamp}.xlsx";
    ...
    // Log error
    error_log("Error exporting maintenance requests: " . $e->getMessage());
