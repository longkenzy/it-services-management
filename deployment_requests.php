                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody id="deployment-requests-table">
                                ...
                                <tr data-customer="<?php echo htmlspecialchars($request['customer_name'] ?? ''); ?>"
                                    ...
                                    data-status="<?php echo htmlspecialchars($request['deployment_status'] ?? ''); ?>"
...
$(document).ready(function() {
    ...
    function applyFilters() {
        ...
        $('#deployment-requests-table tr').each(function() {
            var row = $(this);
    ...
    $('#clearFiltersBtn').on('click', function() {
        ...
        var totalRows = $('#deployment-requests-table tr').length;
        $('#deployment-requests-table tr').show();
        $('.table-responsive').show();
        $('#emptyStateMessage').remove();
    ...
    function showEmptyState() {
        ...
        if (hasActiveFilters) {
            ...
            emptyStateHtml = '<div class="text-center py-5" id="emptyStateMessage">' +
                ...
                '<button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDeploymentRequestModal">' +
                    '<i class="fas fa-plus me-2"></i>' +
                    'Tạo yêu cầu mới' +
                '</button>' +
            '</div>';
            ...
            emptyStateHtml = '<div class="text-center py-5" id="emptyStateMessage">' +
                ...
                '</div>' +
                '<h4 class="text-muted mb-3">Chưa có yêu cầu triển khai nào</h4>' +
                '<p class="text-muted mb-4">Bấm nút "Tạo yêu cầu triển khai" để bắt đầu</p>' +
                '<button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addDeploymentRequestModal">' +
                    '<i class="fas fa-plus me-2"></i>' +
                    'Tạo yêu cầu triển khai' +
                '</button>' +
    ...
    function exportDeploymentRequestsToExcel() {
        ...
        var headers = [
            ...
            'Thời hạn triển khai', 'Ghi chú', 'Tổng số case', 
            'Tổng số task', 'Tiến độ (%)', 'Trạng thái triển khai'
        ];
        data.push(headers);
        ...
        ws['!cols'] = [
            ...
            { width: 25 },  // Khách hàng
            { width: 20 },  // Phụ trách
            { width: 20 },  // Thời hạn triển khai
            { width: 30 },  // Ghi chú
            { width: 12 },  // Tổng số case
            { width: 12 },  // Tổng số task
            { width: 12 },  // Tiến độ (%)
            { width: 15 }   // Trạng thái triển khai
        ];
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Yêu cầu triển khai");
